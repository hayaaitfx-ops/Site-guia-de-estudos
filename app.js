...
// --- SISTEMA DE XP E NÍVEIS CORRIGIDO (ANTI-SPAM DE LEVEL UP) ---
function awardXP(amount, reason, isDeduction = false) {
    ...
    if (isDeduction) {
        ...
        state.dailyXPEarned = Math.max(0, state.dailyXPEarned - amount); 
    } else {
    ...
    // Garante que a propriedade de nível máximo exista no estado do usuário
    if (!state.maxLevelReached) {
        state.maxLevelReached = state.level || 1;
    }

    // Cálculo matemático do nível atual (1 nível a cada 100 XP)
    let correctLevel = Math.floor(state.xp / 100) + 1;
    
    // Lógica de subida e descida de nível
    if (correctLevel > state.level) {
        state.level = correctLevel;
        
        // SÓ COMEMORA SE FOR UM NÍVEL INÉDITO (Impede o spam de ficar marcando/desmarcando a caixa)
        if (state.level > state.maxLevelReached) {
            state.maxLevelReached = state.level; // Atualiza o recorde do usuário
            
            setTimeout(() => {
                // Verifica se as funções existem antes de chamar para evitar erros
                if (typeof playLevelUpChime === 'function') playLevelUpChime();
                if (typeof showLevelUpNotification === 'function') showLevelUpNotification(state.level);
            }, 800);
        }
    } else if (correctLevel < state.level) {
        // Rebaixa o nível atual silenciosamente, mas NÃO altera o maxLevelReached
        state.level = correctLevel;
    }
    ...
    activeRoadmap.steps.forEach((step, index) => {
        ...
        checkbox.onchange = (e) => {
            ...
            if (step.completed) {
                ...
                // AQUI FOI CORRIGIDO: Usa o motor informando que é uma dedução (isDeduction = true)
                awardXP(20, "Módulo desmarcado", true);
            }
            renderRoadmaps();
        };
...
function showQuizResult() {
    ...
    // --- BLOQUEIO DE FARM DE XP NO QUIZ ---
    const todayStr = new Date().toLocaleDateString('pt-BR');

    // 1. Verifica se o usuário já concluiu ESSE quiz hoje (antes de salvar a nova rodada)
    const alreadyDoneToday = (state.quizHistory || []).some(h => h.subjectId === subjectId && h.date === todayStr);

    // 2. Salva no histórico (se já fez hoje, o XP fica zerado no histórico para não bugar os gráficos)
    if (!state.quizHistory) state.quizHistory = [];
    state.quizHistory.push({
        subjectId,
        score: correctCount,
        total,
        xpEarned: alreadyDoneToday ? 0 : xpEarned,
        isPerfect,
        date: todayStr
    });

    // 3. Dá o XP apenas se for a primeira vez no dia
    if (!alreadyDoneToday) {
        if (xpEarned > 0) awardXP(xpEarned, `Quiz: ${quizState.subjectName}`);
        if (isPerfect) awardXP(50, 'Bônus: Quiz Perfeito!');
        
        unlockAchievement('quiz_champion');
        if (isPerfect) unlockAchievement('quiz_perfect');
    } else {
        // Zera visualmente a pontuação na tela de resultados
        quizState.xpEarned = 0; 
        setTimeout(() => alert("Você já pegou o XP deste Quiz hoje! Sua pontuação foi salva, mas o bônus diário já foi resgatado."), 500);
    }
...
// --- Gerenciamento de Matérias Customizadas ---

// 1. Criar nova matéria
document.getElementById('add-subject-btn')?.addEventListener('click', () => {
    ...
    // Cria um ID único e amigável (ex: "História" -> "historia-12345")
    const subjectId = subjectName.toLowerCase().normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/[^a-z0-9]/g, '') + '-' + Date.now();

    // Recupera as matérias existentes (se houver) no estado
    if (!state.customQuizzes) state.customQuizzes = {};

    // Adiciona a nova matéria
    state.customQuizzes[subjectId] = {
        id: subjectId,
        title: subjectName,
        emoji: '📚' // Emoji padrão
    };
    ...
    input.value = '';
    saveToLocalStorage(); // Mudado de saveState() para a função correta do seu app
    
    // Atualiza as listas dropdown e a tela principal
    renderCustomSubjects();
    updateAdminQuizDropdowns(); 
    renderQuiz(); // Atualiza a tela de seleção de Quizzes
});

// 2. Renderizar lista na área de Admin e botão de excluir
function renderCustomSubjects() {
    const list = document.getElementById('custom-subjects-list');
    if (!list) return;
    list.innerHTML = '';

    if (!state.customQuizzes) return;

    Object.keys(state.customQuizzes).forEach(subjectId => {
        const subject = state.customQuizzes[subjectId];
        
        // Pega as perguntas do banco geral correspondentes a esta matéria
        const bank = loadQuizBank().filter(q => q.subjectId === subjectId);
        ...
        li.innerHTML = `
            <span>${subject.title} (${bank.length} questões)</span>
            <div>
                <button onclick="deleteCustomSubject('${subjectId}')" class="btn-secondary" style="padding: 0.2rem 0.5rem; font-size: 0.8rem; color: #ef4444; border: 1px solid rgba(239,68,68,0.3);">Excluir Matéria</button>
            </div>
        `;
        list.appendChild(li);
    });
...
// 3. Excluir matéria e suas perguntas do banco
window.deleteCustomSubject = function(subjectId) {
    if (confirm("Tem certeza que deseja excluir esta matéria e TODAS as suas perguntas do banco?")) {
        // Remove do estado de matérias customizadas
        delete state.customQuizzes[subjectId];
        saveToLocalStorage();
        
        // Remove as perguntas do banco geral
        let bank = loadQuizBank();
        bank = bank.filter(q => q.subjectId !== subjectId);
        saveQuizBank(bank);
...
// 4. Injetar as matérias customizadas nos Selects do Admin e na tela do Aluno
function updateAdminQuizDropdowns() {
    const select = document.getElementById('edit-quiz-subject');
    if (!select) return;

    // Guarda a opção "Nova Matéria"
    const newSubjOption = `<option value="new-subject">* Criar Nova Matéria...</option>`;
    
    // Opções base (vindas do HTML original)
    let optionsHtml = `
        <option value="quiz-webdev">💻 Desenvolvimento Web</option>
        <option value="quiz-python">🐍 Programação Python</option>
        <option value="quiz-math">📐 Matemática Aplicada</option>
        <option value="quiz-english">🇺🇸 Inglês Técnico</option>
        <option value="quiz-logic">🧩 Lógica de Programação</option>
    `;

    // Adiciona as opções customizadas
    if (state.customQuizzes) {
        Object.keys(state.customQuizzes).forEach(id => {
            const subj = state.customQuizzes[id];
            optionsHtml += `<option value="${id}">${subj.emoji} ${subj.title}</option>`;
        });
    }

    select.innerHTML = optionsHtml + newSubjOption;
}

// 5. Sobrescrever a função getQuizSubjects (que alimenta a tela principal) para incluir as criadas
const originalGetQuizSubjects = window.getQuizSubjects;
window.getQuizSubjects = function() {
    const bankSubjects = originalGetQuizSubjects ? originalGetQuizSubjects() : [];
    
    // Filtra as matérias do banco geral para evitar duplicação (caso a lógica original já as pegue)
    const combinedSubjects = [...bankSubjects];
    const existingIds = new Set(combinedSubjects.map(s => s.id));
        ...
        Object.keys(state.customQuizzes).forEach(id => {
            if (!existingIds.has(id)) {
                combinedSubjects.push({
                    id: id,
                    name: state.customQuizzes[id].title,
                    emoji: state.customQuizzes[id].emoji
                });
            }
        });
    }
...
// Inicialização
document.addEventListener('DOMContentLoaded', () => {
    if(typeof loadFromLocalStorage === 'function') loadFromLocalStorage(); // Garante carregamento
    renderCustomSubjects();
    updateAdminQuizDropdowns();
});

// =========================================================================
// CORREÇÃO DEFINITIVA DE ESTILOS DOS MODAIS (INJEÇÃO DINÂMICA)
// =========================================================================
function aplicarEstilosCorrigidosNosModais() {
    // 1. Criar um elemento <style>
    const styleEl = document.createElement('style');
    
    // 2. Definir as regras CSS com prioridade máxima
    styleEl.innerHTML = `
        /* Força fundo escuro em todos os campos de modais */
        .modal-body input,
        .modal-body select,
        .modal-body textarea,
        .modal-content input,
        ...
        .admin-guide-modal input,
        .admin-guide-modal select,
        .admin-guide-modal textarea {
            background-color: rgba(255, 255, 255, 0.05) !important;
            color: #f3f4f6 !important;
            border: 1px solid rgba(255, 255, 255, 0.1) !important;
            border-radius: 8px !important;
            padding: 10px !important;
            appearance: none !important; /* Remove estilo nativo */
            -webkit-appearance: none !important;
        }

        /* Opções do Select escuras */
        .modal-body select option,
        .admin-guide-modal select option {
            background-color: #1a1c2d !important;
            color: #f3f4f6 !important;
        }

        /* Corrige o Layout esmagado (Flexbox Wrap) */
        .admin-guide-modal form,
        .modal-body form {
        ...
        .admin-guide-modal form > div[style*="display: flex"],
        .modal-body form > div[style*="display: flex"] {
            flex-wrap: wrap !important;
            gap: 15px !important;
        }

        .admin-guide-modal .form-group,
        .modal-body .form-group {
            flex: 1 1 200px !important; /* Cresce, encolhe, mínimo 200px */
            display: flex !important;
            flex-direction: column !important;
        }

        .admin-guide-modal label,
        .modal-body label {
    ...
    // 3. Injetar no cabeçalho do documento (só aplica se já não existir)
    if (!document.getElementById('css-injetado-modais')) {
        styleEl.id = 'css-injetado-modais';
        document.head.appendChild(styleEl);
    }
}

// Chamar a função imediatamente quando o JS carregar e após a página estar pronta
aplicarEstilosCorrigidosNosModais();
document.addEventListener('DOMContentLoaded', aplicarEstilosCorrigidosNosModais);

// --- CARREGADOR DINÂMICO DA FOTO DE PERFIL (AGORA ISOLADO POR USUÁRIO) ---
function loadProfilePictureOnDashboard() {
    // Busca a foto usando o e-mail do usuário como chave
    const picKey = getUserDataKey('foto'); 
    ...
    if (!savedPic) {
        // Se esse usuário não tem foto, remove qualquer imagem que tenha ficado de outra conta
        if (sidebarAvatar) sidebarAvatar.style.backgroundImage = 'none';
        if (navAvatar) navAvatar.style.backgroundImage = 'none';
        return;
    }

    // Se tem foto, aplica normalmente
    if (sidebarAvatar) {
        sidebarAvatar.style.backgroundImage = `url(${savedPic})`;
        sidebarAvatar.style.backgroundSize = 'cover';
        sidebarAvatar.style.backgroundPosition = 'center';
        sidebarAvatar.style.setProperty('color', 'transparent', 'important');
        sidebarAvatar.style.setProperty('font-size', '0px', 'important');
    }
    ...
    if (navAvatar) {
        ...
        navAvatar.style.setProperty('color', 'transparent', 'important');
        navAvatar.style.setProperty('font-size', '0px', 'important');
    }
}

// Mantém as execuções normais abaixo da função
loadProfilePictureOnDashboard();
document.addEventListener('DOMContentLoaded', loadProfilePictureOnDashboard);

// =========================================================================
// CORREÇÃO: ISOLAMENTO DE DADOS POR USUÁRIO (MULTICONTAS)
// =========================================================================

// 1. Função para descobrir de quem é a conta atual
function getUserDataKey(type = 'state') {
    try {
        const session = JSON.parse(localStorage.getItem('currentSession') || '{}');
        const email = session.email || 'visitante';
        // Cria uma chave única. Ex: guia_state_admin@gmail.com
        return `guia_${type}_${email}`; 
    } catch (e) {
        return `guia_${type}_visitante`;
    }
}

// 2. Salva os dados na "gaveta" específica desse usuário
window.saveToLocalStorage = function() {
    const key = getUserDataKey('state');
    localStorage.setItem(key, JSON.stringify(state));
};

// 3. Carrega os dados ou ZERA TUDO se for um usuário novo
window.loadFromLocalStorage = function() {
    const key = getUserDataKey('state');
    const savedData = localStorage.getItem(key);
    
    if (savedData) {
        // Se a conta já existe e tem dados, puxa eles pro sistema
        const parsedData = JSON.parse(savedData);
        Object.assign(state, parsedData); 
    } else {
        // SE FOR CONTA NOVA: Obriga o sistema a zerar para não herdar de outra conta!
        state.xp = 0;
        state.level = 1;
        ...
        // Desmarca todos os módulos das trilhas de estudo
        if (state.roadmaps) {
            state.roadmaps.forEach(roadmap => {
                if (roadmap.steps) {
                    roadmap.steps.forEach(step => step.completed = false);
                }
            });
        }
        
        // Salva esse perfil zerado e limpo na nova conta
        saveToLocalStorage();
    }
};

// Opcional, mas recomendado: Limpar resíduos visuais no Logout
document.getElementById('logout-btn')?.addEventListener('click', () => {
    localStorage.removeItem('currentSession'); // Remove a sessão
    window.location.href = 'pagina_login.html'; // Manda pro login
});